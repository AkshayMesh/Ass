...
fun MyModelScreen(
    ...
    val basicState by viewModel.basicState.collectAsStateWithLifecycle()
    if (basicState is BasicUiState.Success) {
        if ((basicState as BasicUiState.Success).data){
            Box(
    ...
    } else if (basicState is BasicUiState.Error){
        (basicState as BasicUiState.Error).throwable.printStackTrace()
        Column(modifier.safeDrawingPadding()) {
            Text(
                "Error"+(basicState as BasicUiState.Error).throwable.message,
...
//            modifier = modifier,
//        )
//    } else if (items is MyModelUiState.Error){
//
//    }
}
